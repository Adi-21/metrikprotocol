...
export function useOneClickBorrow(wallets?: any[]) {
  ...
  const executeOneClickBorrow = useCallback(async (
    ...
    try {
      ...
      toast.info('⏳ Confirming NFT approval on Avalanche blockchain... This may take 30-60 seconds.', { autoClose: 8000 });
      
      if (publicClient && approvalTx.hash) {
        console.log('⏱️ Waiting for ACTUAL approval transaction confirmation on Avalanche testnet...');
        console.log('📋 Transaction hash:', approvalTx.hash);
        
        try {
          const receipt = await publicClient.waitForTransactionReceipt({ 
            hash: approvalTx.hash as `0x${string}`,
            timeout: 60000 // 60 second timeout for Avalanche testnet
          });
      ...
      toast.success(
        ...
        { 
          autoClose: 10000,
          onClick: () => {
            window.open(`https://testnet.snowtrace.io/tx/${borrowTx.hash}`, '_blank');
          }
        }
      );
